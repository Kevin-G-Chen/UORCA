    ...
    try:
        ...
        for gene1 in correlation_matrix.columns:
            ...
            for gene2 in correlation_matrix.columns:
                ...
                correlation_dict[gene1][gene2] = None if pd.isna(corr_val) else round(float(corr_val), 3)
    ...
    for gene in genes:
        ...
        if len(gene_subset) == 0:
            ...
            variability_stats.append({
                ...
                "mean_logFC": round(logfc, 3),
                "median_logFC": round(logfc, 3),
                "min_logFC": round(logfc, 3),
                "max_logFC": round(logfc, 3),
                "contrast_count": 1,
            ...
            variability_stats.append({
                "gene": gene,
                "std_dev": round(float(logfc_values.std()), 3),
                "mean_logFC": round(float(logfc_values.mean()), 3),
                "median_logFC": round(float(logfc_values.median()), 3),
                "min_logFC": round(float(logfc_values.min()), 3),
                "max_logFC": round(float(logfc_values.max()), 3),
