...
class ReflectionResult(BaseModel):
    """Schema for the reflection agent's output."""
    additional_contrasts: List[Contrast_format]
    reflection_summary: str = Field(
        description="Summary of the reflection process, explaining why additional contrasts were suggested or why no more are needed."
    )
    model_config = ConfigDict(extra="allow")
...
# ----------------------------
# Create reflection agent with a focused prompt
# ----------------------------

# Define system prompt for the reflection agent
system_prompt_path = "../script_development/prompt_development/reflection_agent_system_prompt.txt"
try:
    with open(system_prompt_path, 'r') as f:
        system_prompt = f.read()
...
async def process_metadata(ctx: RunContext[RNAseqData]) -> dict:
    ...
    try:
        ...
        raw_df = read_metadata(ctx.deps.metadata_path)
        proc_df = process_dataframe(raw_df)
        final_df = clean_frame(proc_df, ctx)
        
        if final_df.empty:
            raise ValueError("No data after processing")
            
        # Store processed data
        ctx.deps.metadata_df = final_df
        
        # Generate summary
        nrows, ncols = df.shape
        col_names = [str(col) for col in df.columns]
        summary = f"Processed {nrows} rows, {ncols} columns\nColumns: {', '.join(col_names)}"

        log_tool_result(summary)
        return {
            "message": summary,
            "columns": col_names,
            "column_stats": get_stats(df),
            "shape": (nrows, ncols),
            "error": False
        }

    except Exception as err:
        error_msg = f"Error processing metadata: {str(err)}"
...
def run_reflection_process(metadata_deps, initial_contrasts):
    ...
    while True:
        ...
        new_contrasts = []
        for contrast in reflection_result.data.additional_contrasts:
        ...
        meta_this_iter = {
            ...
            "summary": reflection_result.data.reflection_summary
        }
