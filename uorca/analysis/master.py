...
from uorca.shared import RNAseqCoreContext, AnalysisContext, CheckpointStatus
from uorca.shared.workflow_logging import log_tool, setup_logging
from uorca.shared.entrez_utils import configure_entrez
...
# ── load system prompt ──────────────────────────────────────────────────────
master_prompt_path = pathlib.Path(__file__).parent / "prompts" / "master.txt"
try:
...
async def extract(ctx: RunContext[RNAseqCoreContext]) -> str:
    ...
    import importlib
    extraction = importlib.import_module("uorca.analysis.agents.extraction")
...
async def evaluate_analysis(ctx: RunContext[RNAseqCoreContext]) -> str:
    ...
    if not hasattr(ctx.deps, 'checkpoints') or ctx.deps.checkpoints is None:
        from uorca.shared import AnalysisCheckpoints
...
async def analyse(ctx: RunContext[RNAseqCoreContext]) -> str:
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            import importlib
            analysis = importlib.import_module("uorca.analysis.agents.analysis")
