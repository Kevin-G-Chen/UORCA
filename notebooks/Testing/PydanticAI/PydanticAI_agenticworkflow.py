...
if not openai_api_key:
    print("Warning: OPENAI_API_KEY environment variable not found. Please set it to use the OpenAI API.")
...
try:
    ...
    print("OpenAI API test successful:", response.choices[0].message.content[:50] + "...")
...
class UORCAConfig(BaseModel):
    ...
    # API settings
    openai_model: str = "gpt-4o"  # You can replace with your model choice
    ...
    # Test mode settings
    test_mode: bool = False  # When True, limits operations for testing
    test_dataset_limit: int = 2  # Number of datasets to process in test mode
    log_prompts: bool = True  # Log prompts sent to OpenAI
...
# %% pydantic_parser
# Simple implementation of a Pydantic parser to replace langchain functionality
class PydanticOutputParser:
    """Parser that handles JSON output and converts it to a Pydantic model"""

    def __init__(self, pydantic_object):
        self.pydantic_object = pydantic_object

    def parse(self, text):
        """Parse the output and return a Pydantic object"""
    ...
    def get_format_instructions(self):
        """Return the format instructions for the model"""
...
class BaseAgent(BaseModel):
    ...
    def initialize_llm(self, model_name: str = None):
        ...
        # Use direct OpenAI client instead of langchain wrapper
        self.llm = OpenAI(api_key=openai_api_key)

        # Store model name and temperature for use in API calls
    ...
    def chat_completion(self, messages):
        ...
        if config.log_prompts:
            logger.info(f"Agent {self.name} sending prompt to {self.model_name}:")
            for idx, msg in enumerate(messages):
                logger.info(f"Message {idx+1} ({msg.get('role', 'user')}): {msg.get('content', '')[:200]}...")

        try:
            formatted_messages = [{"role": m.get("role", "user"), "content": m.get("content", "")}
                                 for m in messages]
            ...
            # Log a preview of the response if configured
            if config.log_prompts:
...
class ResearchQuestionAgent(BaseAgent):
    ...
    def analyze_question(self, question_text: str) -> ResearchQuestion:
        ...
        # Format the prompt for direct OpenAI API call
        system_prompt = f"""You are an expert in biomedical research. Your task is to analyze a research question and
            ...
            Format your response according to this JSON schema: {parser.get_format_instructions()}
            """

        messages = [
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": f"Research Question: {question_text}\n\nPlease analyze this question and provide a structured response."}
        ]
        ...
        try:
            research_question = parser.parse(response_content)
...
class DatasetFinderAgent(BaseAgent):
    ...
    def evaluate_dataset_relevance(self, dataset_summary: Dict, research_question: ResearchQuestion) -> Tuple[float, str]:
        ...
        system_prompt = """You are an expert in bioinformatics and genomics.
        ...
        user_prompt = f"""Research Question: {research_question.question} (Focuses on: {disease_contexts})
...
class DatasetAnalyzerAgent(BaseAgent):
    ...
    def identify_condition_groups(self, samples: List[SampleMetadata]) -> Dict[str, List[str]]:
        ...
        system_prompt = """You are an expert in bioinformatics and experimental design.
        ...
        user_prompt = f"""Here are the RNA-seq samples from a GEO dataset:
    ...
    def generate_analysis_code(self, analysis_design: AnalysisDesign) -> str:
        ...
        system_prompt = """You are an expert bioinformatician who specializes in RNA-seq analysis.
        ...
        user_prompt = f"""Please generate a code snippet for differential expression analysis with this design:

            Dataset: {analysis_design.dataset_id}
            Control group: {analysis_design.control_group}
            Experimental group: {analysis_design.experimental_group}
            Sample mapping:
            {json.dumps(analysis_design.sample_mapping, indent=2)}
...
class InsightsGeneratorAgent(BaseAgent):
    ...
    def generate_insights(
        ...
        system_prompt = """You are an expert in bioinformatics and systems biology.
        ...
        user_prompt = f"""Research Question: {research_question.question}
            ...
            Common DEGs across datasets: {", ".join(collection.common_degs[:20])}
...
class UORCAWorkflowCoordinator:
    ...
    def run_workflow(self, question_text: str) -> InsightCollection:
        ...
        logger.info(f"Starting workflow for question: {question_text}")

        # Apply test mode settings if enabled
        max_datasets = self.config.test_dataset_limit if self.config.test_mode else self.config.max_datasets
        logger.info(f"Using max_datasets = {max_datasets} (test_mode = {self.config.test_mode})")

        # Step 1: Analyze the research question
        logger.info("Step 1: Analyzing research question")
        ...
        logger.info(f"Step 3: Analyzing {len(datasets)} datasets")
        dataset_results = []
        for i, dataset in enumerate(datasets):
            logger.info(f"Analyzing dataset {i+1}/{len(datasets)}: {dataset.geo_accession} (relevance: {dataset.relevance_score:.2f})")
            ...
            if metadata:
                ...
                logger.info(f"Dataset {dataset.geo_accession} analyzed: {len(samples)} samples, {len(analysis_designs)} analysis designs")
            else:
                logger.warning(f"Failed to analyze dataset {dataset.geo_accession}")

        # Here we'd normally execute the analyses, but we'll simulate results for this prototype
        logger.info("Simulating analysis results")
        simulated_results = self.simulate_analysis_results(dataset_results)
        logger.info(f"Generated {len(simulated_results)} simulated analysis results")

        # Step 4: Generate insights
        logger.info("Step 4: Generating insights from analysis results")
    ...
    def save_results(
        ...
        with open(f"{output_dir}/research_question.json", "w") as f:
            f.write(research_question.model_dump_json(indent=2))

        # Save datasets
        datasets = [r["metadata"] for r in dataset_results]
        with open(f"{output_dir}/datasets.json", "w") as f:
            datasets_json = [d.model_dump() for d in datasets]
            json.dump(datasets_json, f, indent=2)

        # Save analysis results
        with open(f"{output_dir}/analysis_results.json", "w") as f:
            results_json = [r.model_dump() for r in analysis_results]
...
# %% test_workflow
def run_test_workflow(test_mode=True, max_datasets=2, question_index=0):
    """Run a test workflow with a sample research question

    Args:
        test_mode (bool): If True, limits operations for faster testing
        max_datasets (int): Maximum number of datasets to analyze in test mode
        question_index (int): Index of test question to use (0-2)
    """
    # Configure test settings
    config.test_mode = test_mode
    config.test_dataset_limit = max_datasets
    ...
    # Run the workflow for the selected question
    question = test_questions[question_index]
    print(f"Running test workflow for question: {question}")
    print(f"Test mode: {test_mode}, Max datasets: {max_datasets}")
...
if __name__ == "__main__":
    # Run the test workflow with test mode enabled (limits to 2 datasets by default)
    # You can customize these parameters as needed
    insights = run_test_workflow(
        test_mode=True,       # Set to True for faster testing with fewer datasets
        max_datasets=2,       # Maximum number of datasets to analyze in test mode
        question_index=0      # Which test question to use (0-2)
    )
