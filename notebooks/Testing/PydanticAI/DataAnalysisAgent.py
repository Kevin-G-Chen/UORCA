...
if (tx2gene_exists) {{
  ...
  tx2gene <- tx2gene[, c(1, 2)]  # Use first two columns
  ...
  # Without tx2gene, we'll work at transcript level
  # Import Kallisto results at transcript level
...
# Set the reference level for the contrast
dds$group <- relevel(dds$group, ref="{denominator}")

# Run DESeq2
dds <- DESeq(dds)

# Get results for contrast
...
cat("Upregulated in {numerator}:", up_genes, "\\n")
cat("Downregulated in {numerator}:", down_genes, "\\n")
